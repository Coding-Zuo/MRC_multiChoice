...
def pad_to_window_size(input_ids: torch.Tensor, attention_mask: torch.Tensor,
    ...
    '''
    # input_ids: torch.Size([8, 4, 1024])
    # attention_mask : torch.Size([8, 4, 1024])
    w = 2 * one_sided_window_size  # 2*512
    # seqlen = input_ids.size(1) # 4
    seqlen = input_ids.size(2)  # 4
    ...
    F.pad中第二个参数pad只定义两个参数，表示只对输入矩阵的最后一个维度进行扩充，不会对前两个维度造成任何影响，所以此处直接忽略前两个维度。
    (0, padding_len) (左边填充数, 右边填充数)
    https://blog.csdn.net/jorg_zhao/article/details/105295686
    """
    input_ids = F.pad(input_ids, (0, padding_len), value=pad_token_id)  # 描述了填充输入的某些维度的填充大小，从最后一个维度开始并向前移动
